...
Pages.practice = function(params){
  ...
  function _getCardCat(deck, c){
    if(deck.type==='kana')   return c.group||'gojuon';
  ...
  function _buildUI(card){
    ...
    '</div>';
    // category button group — same multi-select behavior as mastery filter
    var catBtns = '<div class="btn-group cat-btn-group">'+
      '<button class="cat-filter-btn filter-btn'+(catAllMode?' active':'')+'" data-cat="all">All</button>'+
      cats.map(function(c){
        var on = !catAllMode && catFilter.indexOf(c)>=0 ? ' active' : '';
        return '<button class="cat-filter-btn filter-btn'+on+'" data-cat="'+escHtml(c)+'">'+escHtml(c)+'</button>';
      }).join('')+
    '</div>';
    ...
    if(!card){
      ...
      cardBlock =
        ...
        '<div class="practice-nav">'+
          ...
          '<span class="card-counter">'+(qIdx+1)+' / '+queue.length+'</span>'+
    ...
    return '<div class="practice-layout">'+
      '<div class="practice-controls-top">'+
        ...
        mfBtns+
      '</div>'+
      '<div class="practice-controls-cats">'+catBtns+'</div>'+
      cardBlock+
    '</div>';
  }
  ...
  function _wireUI(card){
    ...
    var catGroup = document.querySelector('.cat-btn-group');
    if(catGroup){
      catGroup.querySelector('[data-cat="all"]').addEventListener('click', function(){
        catAllMode = true;
        catFilter  = [];
        buildQueue(); isFlipped=false; pendingLevel=null;
        render();
      });
      catGroup.querySelectorAll('.cat-filter-btn:not([data-cat="all"])').forEach(function(btn){
        btn.addEventListener('click', function(){
